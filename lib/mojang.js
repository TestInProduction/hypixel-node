...
class Mojang {
    ...
    constructor() {
        ...
        this.getUUID = (username, callback) => this.request(`${HOST}/users/profiles/minecraft/${username}`, 'id', callback);

        this.getName = (uuid, callback) => this.request(`https://sessionserver.mojang.com/session/minecraft/profile/${clean(uuid)}`, 'name', callback);
        
        this.getStatus = (callback) => this.request('https://status.mojang.com/check', null, callback);

        this.getNameHistory = (search, callback) => {
            if(isUUID(search))
